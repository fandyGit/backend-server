...
module.exports.remove=function (obj,cb) {
  mconnection(collections=>{
    collections.remove(obj,(err,data)=>{
      cb(data);
    })
  })
}
...
* @params limit 跳过的个数
* */
module.exports.skip=function (skip,limit,cb) {
  mconnection(collections=>{
    collections.find().skip(skip).limit(limit).toArray((err,data)=>{
      cb(data);
    })
  })
}
...
module.exports.count=function (cb) {
  mconnection(collections=>{
    collections.find().count().then(data=>{
      cb(data);
    })
  })
}
